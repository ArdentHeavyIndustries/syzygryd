...
#define GROUPS_AS_OBJECTS 1	// added by mike estee, use the 'o' marker instead of the 'g' marker for groups
...
/* glmScale: Scales a group by a given amount.
 * 
 * group - properly initialized GLMgroup structure
 * scale - scalefactor (0.5 = half as large, 2.0 = twice as large)
 */
/*GLvoid
glmScaleGroup(GLMgroup* group, GLfloat scale)
{
	GLuint i;
	
	for (i = 1; i <= group->numtriangles; i++) {
		GLMtriangle *triangle = group->triangles[i];
		
		model->vertices[triangle->vindices[X]] *= scale;
		model->vertices[triangle->vindices[Y]] *= scale;
		model->vertices[triangle->vindices[Z]] *= scale;
	}
}*/
	...
	//glBegin(GL_TRIANGLES);
		...
		while( group )
		{
			...
			if (mode & GLM_MATERIAL)
			{
				...
				model->materials[group->material].emmissive);
			...
			for( i = 0; i < group->numtriangles; i++ )
			{
				...
				// draw each vertex that makes up triangle
				for( v=0; v<3; v++ )
				{
					if (mode & GLM_SMOOTH)
						glNormal3fv(&model->normals[3 * T(group->triangles[i]).nindices[v]]);
					if (mode & GLM_TEXTURE)
						glTexCoord2fv(&model->texcoords[2*T(group->triangles[i]).tindices[v]]);
						glVertex3fv(&model->vertices[3 * T(group->triangles[i]).vindices[v]]);
				}
			}
